package ScalaSpark.FPPrinciple.week3DataAndAbstraction.assignment
...
abstract class TweetSet {
   ...
   */
    def union(that: TweetSet): TweetSet =
       filterAcc(e => this.contains(e),that.filterAcc(e => that.contains(e), new Empty()))
   ...
   * and be implemented in the subclasses?
   */
    def mostRetweeted: Tweet = mostAccum(new Tweet("tmp", "tmp", 0))
    def mostAccum(tmp: Tweet):Tweet
...
class Empty extends TweetSet {
    ...
    def mostAccum(tmp:Tweet):Tweet=tmp
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostAccum(tmp:Tweet):Tweet={
    if (tmp.retweets>elem.retweets) right.mostAccum(left.mostAccum(tmp))
    else right.mostAccum(left.mostAccum(elem))
  }
